<?php
/**
 * Full-width lesson demo page template.
 *
 * Template Name: Full Width Lesson
 *
 * @package The_Hidden_Word_Theme
 */

get_header();
?>

<main id="primary" class="thw-main thw-main--lesson-full">
	<div class="thw-container thw-container--wide thw-content">
		<?php
		while ( have_posts() ) :
			the_post();
			?>
			<article <?php post_class( 'thw-article thw-article--lesson-full' ); ?>>
				<?php if ( ! is_front_page() ) : ?>
					<header class="thw-article__header">
						<h1 class="thw-article__title"><?php the_title(); ?></h1>
					</header>
				<?php endif; ?>
				<div class="thw-article__body entry-content">
					<?php the_content(); ?>
				</div>
			</article>
			<?php
		endwhile;
		?>
	</div>
</main>

<?php
get_footer();
